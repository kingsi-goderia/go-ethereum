...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		log.Info("handler.go: make subprotocol")
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		...
		log.Info("handler.go: Ethereum handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		...
		log.Info("handler.go: Ethereum peer registration failed", "err", err)
		return err
	}
	...
	if err := pm.downloader.RegisterPeer(p.id, p.version, p); err != nil {
		log.Info("handler.go: Ethereum peer registration in downloader failed", "err", err)
		return err
	}
