...
func (m *Message) GetTopic() []byte    { return m.message.Topic[:] }
...
func (c *Criteria) GetTopics() [][]byte {
	topics := make([][]byte, len(c.criteria.Topics))
...
func (c *Criteria) SetTopics(topics [][]byte) {
	topicTypes := make([]whisper.TopicType, len(topics))
	for i, topic := range topics {
		topicTypes[i] = whisper.BytesToTopic(topic)
	}
